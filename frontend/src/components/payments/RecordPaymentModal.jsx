...
export default function RecordPaymentModal({
  ...
  useEffect(() => {
    if (open) {
      ...
      if (preselectedStudent?.id) {
        ...
        if (preselectedStudent.pending !== undefined) {
          setAmount(String(preselectedStudent.pending))
        }
      } else {
    ...
    if (selectedStudentId && open) {
      ...
      api
        ...
        .then((res) => {
          ...
          if (payload?.ledger?.pendingAmount !== undefined) {
            setAmount(String(payload.ledger.pendingAmount))
          } else if (preselectedStudent?.pending !== undefined) {
            setAmount(String(preselectedStudent.pending))
          }
  ...
  const pendingDue = studentLedger?.ledger?.pendingAmount ?? preselectedStudent?.pending ?? 0
  const totalFee = studentLedger?.ledger?.totalFee ?? preselectedStudent?.totalFee ?? 0
  const paidAmount = studentLedger?.ledger?.paidAmount ?? preselectedStudent?.paid ?? 0
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {!preselectedStudent ? (
          ...
          <div className="rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 p-3.5 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                Admission No: <span className="font-mono font-medium text-slate-700 dark:text-slate-300">{preselectedStudent.admissionNo || preselectedStudent.studentCode || preselectedStudent.studentId || preselectedStudent.id}</span> &nbsp;|&nbsp; Class: {preselectedStudent.class || `${preselectedStudent.className || ''} ${preselectedStudent.section || ''}`.trim()}
              </p>
            </div>
          </div>
        )}
        ...
        {selectedStudentId && (
          <div className="grid grid-cols-3 gap-1.5 sm:gap-3 rounded-xl bg-violet-50/70 dark:bg-violet-950/30 border border-violet-100 dark:border-violet-900/50 p-2.5 sm:p-3.5 text-center">
            <div className="min-w-0">
              <span className="text-[9px] sm:text-[10px] uppercase font-bold text-violet-600 dark:text-violet-400 block truncate">Total Fee</span>
              <span className="font-mono font-bold text-slate-900 dark:text-white text-xs sm:text-sm truncate block mt-0.5">
                {formatCurrency(totalFee)}
              </span>
            </div>
            <div className="min-w-0">
              <span className="text-[9px] sm:text-[10px] uppercase font-bold text-emerald-600 dark:text-emerald-400 block truncate">Paid So Far</span>
              <span className="font-mono font-bold text-emerald-600 dark:text-emerald-400 text-xs sm:text-sm truncate block mt-0.5">
                {formatCurrency(paidAmount)}
              </span>
            </div>
            <div className="min-w-0">
              <span className="text-[9px] sm:text-[10px] uppercase font-bold text-rose-600 dark:text-rose-400 block truncate">Remaining Due</span>
              <span className="font-mono font-bold text-rose-600 dark:text-rose-400 text-xs sm:text-sm truncate block mt-0.5">
        ...
        <div className="grid grid-cols-1 gap-3 sm:gap-4 sm:grid-cols-2">
          <Input
            label="Transaction ID (Bank / Cheque No)"
            value={transactionId}
            onChange={(e) => setTransactionId(e.target.value)}
            placeholder="e.g. BNK/2026/89472"
          />
          <Input
