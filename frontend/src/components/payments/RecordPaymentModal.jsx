...
export default function RecordPaymentModal({
  ...
  // Auto-generate transaction ID based on payment method
  useEffect(() => {
    if (open) {
      const isAutoGenerated = !transactionId || /^(CSH|BNK|CHQ|TXN)\/\d{4}\/\d{6}$/.test(transactionId);
      if (isAutoGenerated) {
        ...
        const prefix = prefixMap[paymentMethod] || 'TXN';
        setTransactionId(`${prefix}/${new Date().getFullYear()}/${timestamp}`);
      }
    } else {
      setTransactionId('');
    }
  ...
  // Auto-generate reference number whenever modal opens
  useEffect(() => {
    if (open) {
      const isAutoGenerated = !referenceNumber || /^REF-\d{4}-\d{3}$/.test(referenceNumber);
      if (isAutoGenerated) {
        const seq = Math.floor(100 + Math.random() * 900);
        setReferenceNumber(`REF-${new Date().getFullYear()}-${seq}`);
      }
    } else {
      setReferenceNumber('');
    }
