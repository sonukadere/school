...
export const resetTeacherCredentials = asyncHandler(async (req, res) => {
  const result = await teacherService.resetTeacherCredentials(req.params.id, req.body);
  res.status(200).json(new ApiResponse(200, result.message || 'Teacher credentials updated successfully.', result));
});
